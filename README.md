...
### `moving-main`
For repos where several agents each work in their own worktree and open their own PRs, so the base branch moves while you wait on CI. Most of the lost time there is not merge conflicts, it is trusting a signal computed against a base that no longer exists: a `push` run tests the workflow on your branch while a `pull_request` run tests the workflow from your head merged into the base, so one SHA can produce two runs that disagree. Covers that, plus stale per-worktree asset caches that fail tests which are fine, flaky-or-real triage, working out whether CI actually gates the test a PR adds (hardcoded suite list or auto-discovery, which fail in opposite directions), and the fact that `gh pr merge --delete-branch` can print a git error after the merge already landed.
- `docs/methods/moving-main.md` — the five failure modes with the command that catches each, and cleanup that verifies the merge and the branch deletion separately from the exit code
